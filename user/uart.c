 ...
 * Returns      : OK
*******************************************************************************/
void uart0_tx_one_char(uint8 c) {
	while (((READ_PERI_REG(UART_STATUS(UART0))>>UART_TXFIFO_CNT_S)&UART_TXFIFO_CNT)>=126);
	WRITE_PERI_REG(UART_FIFO(UART0) , c);
}
void uart1_tx_one_char(uint8 c) {
//	while (((READ_PERI_REG(UART_STATUS(UART1)) & (UART_TXFIFO_CNT<<UART_TXFIFO_CNT_S)) >> UART_TXFIFO_CNT_S & UART_TXFIFO_CNT) >= 126);
...
void uart0_write_char(char c) {
	if ('\n' == c) uart0_tx_one_char('\r');
	if ('\r' != c) uart0_tx_one_char(c);
}
void uart1_write_char(char c) {
	if ('\n' == c) uart1_tx_one_char('\r');
	if ('\r' != c) uart1_tx_one_char(c);
}
/******************************************************************************
...
/*void uart_reattach() {
// rom use 74880 baut_rate, reinitialize in uart_init
